...
import { DEEP_TREATMENT_ORIGIN, PROCESS_PUMP_LAYOUT, deepTreatmentLocal } from './processPumpLayout';
import { TANK_LAYOUT } from './tankLayout';

const DEEP_ORIGIN: [number, number, number] = DEEP_TREATMENT_ORIGIN;
...
const INTER_PUMP_1 = deepTreatmentLocal(PROCESS_PUMP_LAYOUT.intermediateA.position);
const INTER_PUMP_2 = deepTreatmentLocal(PROCESS_PUMP_LAYOUT.intermediateB.position);
const DRAIN_PUMP_1 = deepTreatmentLocal(PROCESS_PUMP_LAYOUT.drainA.position);
const DRAIN_PUMP_2 = deepTreatmentLocal(PROCESS_PUMP_LAYOUT.drainB.position);
...
export const DeepTreatmentSection: React.FC<DeepTreatmentSectionProps> = ({ isDrainRunning }) => {
  ...
  return (
    <group position={DEEP_ORIGIN}>
      ...
      <DAFTank3D id="tk-daf" position={deepTreatmentLocal(TANK_LAYOUT['tk-daf'].center)} size={TANK_LAYOUT['tk-daf'].size} />
      <Tank3D id="tk-mixing" position={deepTreatmentLocal(TANK_LAYOUT['tk-mixing'].center)} size={TANK_LAYOUT['tk-mixing'].size} hasAgitator />
      <MixingPHPanel />
      <Tank3D id="tk-drainage" position={deepTreatmentLocal(TANK_LAYOUT['tk-drainage'].center)} size={TANK_LAYOUT['tk-drainage'].size} />

      <Pump3D id={PROCESS_PUMP_LAYOUT.drainA.id} position={DRAIN_PUMP_1} rotation={[0, PROCESS_PUMP_LAYOUT.drainA.rotationY, 0]} />
      <Pump3D id={PROCESS_PUMP_LAYOUT.drainB.id} position={DRAIN_PUMP_2} rotation={[0, PROCESS_PUMP_LAYOUT.drainB.rotationY, 0]} />
