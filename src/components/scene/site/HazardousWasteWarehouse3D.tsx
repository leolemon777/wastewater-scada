...
export const HazardousWasteWarehouse3D: React.FC<{ isNight?: boolean }> = ({ isNight = false }) => {
  ...
  return (
    <group position={position}>
      ...
      {/* Exterior structural columns on the long side walls. */}
      {[-d / 4, 0, d / 4].map((z, index) => (
        <mesh
          key={`hazwaste-east-column-${index}`}
          position={[w / 2 + WALL_THICK / 2 + 0.01, h / 2, z]}
          castShadow
          receiveShadow
        >
      ...
      {[-d / 4, 0, d / 4].map((z, index) => (
        <mesh
          key={`hazwaste-west-column-${index}`}
          position={[-w / 2 - WALL_THICK / 2 - 0.01, h / 2, z]}
          castShadow
          receiveShadow
        >
      ...
      {[-w / 4, w / 4].map((x, index) => (
        <group
          key={`hazwaste-south-louver-${index}`}
          position={[x, h * 0.6, d / 2 + WALL_THICK / 2 + 0.01]}
        >
          <mesh>
            <boxGeometry args={[0.8, 0.6, 0.05]} />
            <meshStandardMaterial color="#374151" roughness={0.8} />
          </mesh>
          {[-0.2, -0.1, 0, 0.1, 0.2].map((y, slatIndex) => (
            <mesh
              key={`hazwaste-louver-slat-${slatIndex}`}
              position={[0, y, 0.03]}
              rotation={[-0.2, 0, 0]}
            >
      ...
      {[-w / 4, w / 4].map((x, index) => (
        <group key={`hazwaste-roof-vent-${index}`} position={[x, h + 0.35, 0]}>
          <mesh castShadow>
