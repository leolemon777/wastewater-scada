...
const MUD_FLAKE_INDICES = [0, 1, 2, 3, 4] as const;
const CLOD_INDICES = [0, 1, 2] as const;
...
const MudFlakes: React.FC = () => {
  const flakes = useRef<Array<THREE.Mesh | null>>([]);
  
  useFrame((state) => {
    flakes.current.forEach((flake, index) => {
      if (flake) {
        ...
        flake.position.y = curY;
        flake.position.x = curX;
        
        // Add subtle horizontal random jitter and spread so it reads like dewatered sludge cakes.
        flake.position.z = Math.sin(index * 1.7 + t * 6) * 0.055;
        
        // Tumble and rotate the chunks realistically as they fall
        flake.rotation.x = t * Math.PI * 2.5 + index;
        flake.rotation.y = t * Math.PI * 1.5 + index;
        
        // Shrink slightly as it enters the bag
        const s = 1.0 - t * 0.15;
        flake.scale.set(s * (1 + (index % 2) * 0.18), s * 0.75, s);
      }
    });
  });
  ...
  return (
    <>
      {MUD_FLAKE_INDICES.map((idx) => (
        <mesh ref={(mesh) => { flakes.current[idx] = mesh; }} key={idx} position={[1.7, -0.42, 0]} castShadow>
...
const SludgeCakeDischarge: React.FC = () => {
  ...
  const clodRefs = useRef<Array<THREE.Mesh | null>>([]);
  ...
  useFrame((state) => {
    ...
    clodRefs.current.forEach((clod, index) => {
      if (!clod) return;
      const t = (state.clock.elapsedTime * 1.2 + index * 0.33) % 1;
      clod.position.y = -0.05 - t * 0.62;
      clod.position.x = RECEIVING_BAG_X + Math.sin(t * 5 + index) * 0.035;
      clod.position.z = RECEIVING_BAG_Z + Math.cos(t * 6 + index) * 0.045;
      clod.rotation.x = t * Math.PI * 3 + index;
      clod.rotation.z = t * Math.PI * 2;
    });
  });
  ...
  return (
    <group>
      ...
      {CLOD_INDICES.map((index) => (
        <mesh key={index} ref={(mesh) => { clodRefs.current[index] = mesh; }} position={[2.2, -0.35, 0]} castShadow>
