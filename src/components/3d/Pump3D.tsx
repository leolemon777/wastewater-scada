...
import { DiegeticPanel3D } from './DiegeticPanel3D';
import { EquipmentNameplate3D, RubberPad3D, PumpIndicator3D } from './IndustrialParts';
...
export const Pump3D: React.FC<Pump3DProps> = ({ id, position, rotation = [0, 0, 0] }) => {
  ...
  useFrame((state, delta) => {
    ...
    if (machineRef.current) {
      machineRef.current.position.y = running ? Math.sin(state.clock.elapsedTime * 44) * 0.006 : 0;
    }
  });
