...
const MudFlakes: React.FC = () => {
  ...
  useFrame((state) => {
    flakes.forEach((ref, index) => {
      if (ref.current) {
        ...
        ref.current.position.y = curY;
        ref.current.position.x = curX;
        
        // Add subtle horizontal random jitter and spread so it reads like dewatered sludge cakes.
        ref.current.position.z = Math.sin(index * 1.7 + t * 6) * 0.055;
        
        // Tumble and rotate the chunks realistically as they fall
        ref.current.rotation.x = t * Math.PI * 2.5 + index;
        ref.current.rotation.y = t * Math.PI * 1.5 + index;
        
        // Shrink slightly as it enters the bag
        const s = 1.0 - t * 0.15;
        ref.current.scale.set(s * (1 + (index % 2) * 0.18), s * 0.75, s);
      }
    });
  });
  ...
  return (
    <>
      {flakes.map((ref, idx) => (
        <mesh ref={ref} key={idx} position={[1.7, -0.42, 0]} castShadow>
...
const SludgeCakeDischarge: React.FC = () => {
  ...
  const clodRefs = [useRef<THREE.Mesh>(null), useRef<THREE.Mesh>(null), useRef<THREE.Mesh>(null)];
  ...
  useFrame((state) => {
    ...
    clodRefs.forEach((ref, index) => {
      if (!ref.current) return;
      const t = (state.clock.elapsedTime * 1.2 + index * 0.33) % 1;
      ref.current.position.y = -0.05 - t * 0.62;
      ref.current.position.x = RECEIVING_BAG_X + Math.sin(t * 5 + index) * 0.035;
      ref.current.position.z = RECEIVING_BAG_Z + Math.cos(t * 6 + index) * 0.045;
      ref.current.rotation.x = t * Math.PI * 3 + index;
  ...
  return (
    <group>
      ...
      {clodRefs.map((ref, index) => (
        <mesh key={index} ref={ref} position={[2.2, -0.35, 0]} castShadow>
