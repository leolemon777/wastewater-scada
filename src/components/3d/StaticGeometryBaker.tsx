...
export const StaticGeometryBaker: React.FC<{ rebuildKey?: string }> = ({ rebuildKey }) => {
  ...
  useFrame(() => {
    // no-op; kept so the component stays inside the R3F render loop lifecycle.
  });
...
/**
 * Helper: mark a group (and its subtree) as off-limits for the static baker.
 * Use on animated assemblies: agitator shaft/impeller, pump fan/coupling, the
 * scaling water body, selection outlines, status rings, animated flow meshes.
 *
 *   <group ref={agitatorRef} userData={{ bakeExclude: true }}> ...
 */
export function markBakeExclude(obj: THREE.Object3D | null) {
  if (obj) obj.userData.bakeExclude = true;
}
