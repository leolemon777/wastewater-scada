...
1. `src/components/3d/pipes/pumpPorts.ts`：`FLANGE_FACE_INSET` −0.025 → **0.06**。正值让 face 在 discharge 上方，经 equipment 延伸 0.12 后立管底端 = discharge − 0.06 = 1.28，落入出水主体 [1.24, 1.34] 被遮挡。
2. `src/components/3d/pipes/Pipe3D.tsx`：`BEND_RADIUS_MULTIPLIER` 2.6 → **1.0**（1D 标准弯头）。
3. `src/components/3d/pipes/Pipe3D.tsx`：`buildRoundedPath` 腿长倒角上限 0.42 → **0.15**（两处）。0.55 m 竖直段中间保留 0.385 m 直线。
