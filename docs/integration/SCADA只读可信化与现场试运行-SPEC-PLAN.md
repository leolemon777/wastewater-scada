...
>
> **WP6.7 Baker 修复（2026-08-21，第一阶段）：** 诊断发现运行时 Baker 实际生效但分组签名把颜色算进桶键（314 桶）；而烘焙材质本就是 vertexColors（真实颜色已在顶点色），颜色分量纯属冗余。修复：签名仅按 PBR 属性（roughness/metalness/透明/自发光/clearcoat）分组 + 材质模板色保持白 + PBR 量化 32→10 级。**实测：烘焙桶 314→52（5567 静态 mesh → 52 draw calls）；六位 quick：全局 5489→5121、污泥 468→350 calls / FPS 39.8、气浮 671→550 / FPS 34.6、主处理 1399→1144、纯水 2083→1770**；视觉回归截图通过。**Stage-2（电机组门控）：** 泵电机总成因亚毫米装饰微震动画被整体排除在 bake 外（每泵数十 mesh × 21+5 台）——perf-mode 下禁用微震（WP6.1 停帧同思路），`bakeExclude` 条件化，电机总成并入静态 bake（5759 mesh → 52 桶）。**全局 FPS 8.4→25.6、P95 33.4ms（达门槛线）、calls 5121→4785；污泥位 326 calls / FPS 24-39（近景 ≤300 门槛区间）；全部六位 calls 均降**（进水 1729→1344、主处理 1401→1144、气浮 671→502、纯水 2083→1699）。视觉回归截图通过、全量回归绿。**Stage-3（ChemicalTank 整机门控）：** 药剂桶整机排除同因保守——perf-mode 下并入 bake（涡流/搅拌轴子组保留自身排除标记），6073 mesh → 54 桶。**默认视口 calls 4785→2170；六位 quick：进水 1344→674 / FPS 25.4、主处理 1144→414（<500 门槛 ✓）/ FPS 29、气浮 502→504 / FPS 27.8、污泥 326→328 / FPS 28.3、纯水 1699→743 / FPS 21.3**。**已知：quick 全局位 P95 被 bake 第二趟重算卡顿污染（~10.6s 主线程阻塞）——FULL 模式 60s×3 采样可摊薄，Baker 增量/异步重算列为 stage-4；perf-mode 下 3D 选择高亮色变不重绘 baked 部件（Tank3D 水体同型折中，详情面板反馈不受影响）。**Stage-3.5（关键修复，用户现场发现颜色全变）：** stage-1 去色在 HQ 同样生效且模板色置白——颜色丢失（全场灰白）。**修复：baker 完全按模式分裂——HQ=WP6.7 前原路径（颜色 16 级入键+PBR 32 级+组模板色+原排除集，252 桶，源码视觉零变化）；PERF=PBR-only（10 级）+逐 mesh 材质色烘顶点色+扩大静态集。** 规则固化：性能实验只影响部署构建（perf-mode），HQ 源码视觉红线。
> **Stage-4（第二趟门控+纯水泵并入）：** 第二趟可见静态新增 <50 跳过全量重算（曾因 8 个小件触发 ~10s 阻塞）；PureWaterPump 整机排除门控 PERF。**PERF 六位 quick（开发机）：进水 40.6 / 主处理 45.0 / 气浮 43.5 / 污泥 48.0 / 纯水 39.1 FPS，P95 全部 33.5-33.7ms——五个近景位全线达 SPEC 门槛；calls 661/379/505/329/670。** 全局位 quick 受第一趟合并污染（3s 预热不足；SPEC FULL 15s 预热设计覆盖，stage-2 实测 bake 后 25.6 FPS/P95 33.4）。遗留：geometry 往返泄漏（量化 +39/轮，kiosk 低频切换下 8h soak 不受影响，验收轮前修）；Baker 异步分块（FULL 预热覆盖）。**
