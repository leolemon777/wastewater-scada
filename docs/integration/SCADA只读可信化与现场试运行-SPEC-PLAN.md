...
>
> **WP6.7 Baker 修复（2026-08-21，第一阶段）：** 诊断发现运行时 Baker 实际生效但分组签名把颜色算进桶键（314 桶）；而烘焙材质本就是 vertexColors（真实颜色已在顶点色），颜色分量纯属冗余。修复：签名仅按 PBR 属性（roughness/metalness/透明/自发光/clearcoat）分组 + 材质模板色保持白 + PBR 量化 32→10 级。**实测：烘焙桶 314→52（5567 静态 mesh → 52 draw calls）；六位 quick：全局 5489→5121、污泥 468→350 calls / FPS 39.8、气浮 671→550 / FPS 34.6、主处理 1399→1144、纯水 2083→1770**；视觉回归截图通过。**遗留：全局 ≤500 门槛未达（剩余大头为管道流动层 ~800 + 设备动画排除组 + 197 InstancedMesh，需流动层合批/动画组细分——下一阶段）；geometry 往返泄漏（WP6.5 发现）未修。**
