...
```text
src/components/scene/sections/IndustrialPipeNetwork3D.tsx
src/components/scene/sections/ProcessAndSludgePipeNetwork3D.tsx
```
...
| PH1 → 芬顿 → PH2 → 混凝 → 絮凝 → 沉淀 → PH3 → 中间池 | 池间溢流 / 土建连通 | 无外部工艺水管 | 相邻段由 `OverflowCascade3D` 表达，其余段按土建溢流通道处理；禁止补画外部闭式工艺水管 |
...
| DAF 浮渣 → 污泥池 | DAF 浮渣泵组 | 污泥池 | 双泵吸入/出水总管，经混合池与排水池之间的高位通廊接入统一接收总管 |
...
- 已移除位置和标高不统一的通用装饰管托；后续只按确定的结构梁位单独恢复工程支撑。
...
```text
src/components/scene/SCADAScene.tsx
```
...
- PH1 → 芬顿 → PH2 → 混凝 → 絮凝 → 沉淀 → PH3 → 中间池整段按溢流/土建连通处理，不画外部闭式工艺水管。
