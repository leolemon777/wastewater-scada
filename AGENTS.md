...
| IP | 现场模块 | 网桥 | 状态 |
| --- | --- | --- | --- |
| `192.168.0.7` | **混合池**(2026-08-19 用户确认故意保留默认 IP,现场唯一 `.7`;SN `...5842` 非原排水池那台) | `.67` | ✅ 在线(127ms);传感器走 RS485 Modbus,ioread 读不到,数据只在有人云 |
...
- 默认管理 IP `192.168.2.66`,账号/密码见本地被忽略的 `docs/integration/本地凭据.local.md`(不入 git)。
