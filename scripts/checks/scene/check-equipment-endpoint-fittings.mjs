...
const NON_PUMP_EQUIPMENT_FITTING_RE =
  /<(?:ChemicalMeteringPump3D|PumpPipeFlanges3D|PipeWallPort3D|PipeOpenFlange3D|DosingPort|CleanWaterHeaderTerminal|OutfallDropNozzle3D)\b/g;
const PUMP_ROUTE_HELPER_RE =
  /\b(?:getDischargeBranch|getSuctionBranch|getDirectTankSuctionBranch|getDischargeRiser)\s*\(/g;
/** Live processPumpRoutes tables — points already terminate on sealing faces. */
const PUMP_ROUTE_TABLE_RE =
  /\broute\.(?:suctionPoints|dischargePoints|wallPoint)\b/g;
...
for (const file of walk(SECTION_DIR)) {
  ...
  for (const pipe of collectPipeBlocks(text)) {
    ...
    const pumpHelperEndpointCount =
      ...
      + (pipe.block.match(PUMP_ROUTE_TABLE_RE) ?? []).length;
    // route.suctionPoints + nearby route.wallPoint cover both equipment ends of a pump suction spool.
    const routeWallInContext = /\broute\.wallPoint\b/.test(context) ? 1 : 0;
    const fittingCount =
      visibleFittingCount + namedFittingCount + sharedFittingCount + pumpHelperEndpointCount + routeWallInContext;
