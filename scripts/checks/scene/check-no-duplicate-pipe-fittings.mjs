...
const SECTIONS_DIR = path.join(ROOT, 'src/components/scene/sections');
...
for (const file of walk(SECTIONS_DIR)) {
  ...
  while ((match = FITTING_COMPONENT_RE.exec(text)) !== null) {
    ...
    // Map/table-driven positions (route.wallPoint, branch.wallPoint, …) expand to
    // distinct world coordinates at runtime — static expression equality is not a
    // real duplicate.
    if (/\b(?:route|branch|pump|group)\./.test(position) || /\.map\s*\(/.test(attrs)) {
      continue;
    }
