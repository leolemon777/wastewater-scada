...
while ((match = FORBIDDEN_CSS_GRADIENT_RE.exec(text)) !== null) {
  issues.push(`src/index.css:${lineNumber(match.index)} decorative CSS gradient "${match[0]}" is not allowed in the SCADA UI shell`);
}

console.log('Decorative CSS gradient guard: scanned src/index.css');
